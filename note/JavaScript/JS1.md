...
**实参过多**：实参的个数多于形参的个数，会从前面开始取，其余的实参被忽略（函数内部有一个arguments，里面装着所有的实参）

**形参过多**：实参的个数少于形参的个数，多的形参默认为undefined
...
```html
  <script>
    ...
    getSum(2, 3) // [] 2给了a 3给了b 无剩余
    getSum(1, 2, 3, 4, 5) // [3, 4, 5] 1给了a 2给了b 剩余给了arr
  </script>
```
